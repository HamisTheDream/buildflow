...
const props = defineProps<{
  ...
  metrics: { total: number; active: number; trial: number; expired: number; deleted: number }
  isSuper: boolean
}>()
...
function confirmDelete() {
  const org = orgToDelete.value
  orgToDelete.value = null
  if (!org) return
  deleteForm.delete(`/owner/organizations/${org.id}`, {
    preserveScroll: true,
  })
}
</script>
  ...
  <OwnerLayout>
    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <Link
          href="/owner/organizations/trash"
        ...
        >
          ...
          Trash
          <span v-if="metrics.deleted > 0" class="inline-flex items-center rounded-full bg-red-100 px-2 py-0.5 text-xs font-semibold text-red-700">
            {{ metrics.deleted }}
          </span>
        </Link>
      </div>
            ...
            <Link :href="`/owner/organizations/${org.id}`" class="block p-6">
                <div class="flex items-center justify-between gap-x-6 sm:items-start">
                    ...
                    <div class="flex flex-col items-end gap-y-2">
                        ...
                        <button
                            v-if="isSuper"
                            @click.stop.prevent="askDelete(org)"
      ...
      <Modal :show="!!orgToDelete" @close="orgToDelete = null">
        <div class="p-6">
          <h2 class="text-lg font-medium text-gray-900">Delete organization?</h2>
          <p class="mt-2 text-sm text-gray-600">
            Are you sure you want to delete
            <span class="font-medium text-gray-900">{{ orgToDelete?.name }}</span>?
            The organization and all of its data (projects, CRM, finance, HR, files)
            will be moved to trash and can be restored. Uploaded files stay intact.
          </p>
          <div class="mt-6 flex justify-end gap-2">
            <SecondaryButton @click="orgToDelete = null">Cancel</SecondaryButton>
