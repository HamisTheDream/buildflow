...
watch(flash, (newFlash) => {
  if (newFlash.success) {
    ...
    show.value = true
    setTimeout(() => { show.value = false }, 12000)
...
<template>
  <div
    ...
    :class="[
      type === 'success' ? 'bg-emerald-50 text-emerald-900 border border-emerald-100' : type === 'warning' ? 'bg-amber-50 text-amber-900 border border-amber-200' : 'bg-red-50 text-red-900 border border-red-100'
    ]"
  >
    <div 
        class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full"
        :class="type === 'success' ? 'bg-emerald-100 text-emerald-600' : type === 'warning' ? 'bg-amber-100 text-amber-600' : 'bg-red-100 text-red-600'"
    >
      <svg v-if="type === 'success'" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
      </svg>
      <svg v-else-if="type === 'warning'" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
